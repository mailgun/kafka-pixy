...
func TestConsumergroupInstances(t *testing.T) {
	...
	if reg.Pattern != RegPatternStatic {
		t.Errorf("Unexpected pattern: %v", reg.Pattern)
	}
	if !reflect.DeepEqual(reg.Subscription, map[string]int{"topic": 1}) {
		t.Errorf("Unexpected subscription: %v", reg.Subscription)
	}
...
// It is perfectly fine to repeat registration for the same
func TestConsumergroupInstancesSameID(t *testing.T) {
	// Given
	kz, err := NewKazoo(zookeeperPeers, nil)
	if err != nil {
		t.Fatal(err)
	...
	_ = instance.Register([]string{"topic"})

	// When
	err = instance.Register([]string{"foo", "bar"})
	if err != nil {
		t.Error(err)
	}

	// Then
	reg, err := instance.Registration()
	if err != nil {
		t.Error(err)
	}
	if !reflect.DeepEqual(reg.Subscription, map[string]int{"foo": 1, "bar": 1}) {
		t.Errorf("Unexpected subscription: %v", reg.Subscription)
	}
